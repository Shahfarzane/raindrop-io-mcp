...
export default function batchTagItems({
	...
	sections.push("");
	sections.push(
		`You need to analyze ${items.length} bookmarks and generate tags for each.`,
	);
	...
	sections.push(
		"2. **Use lowercase, hyphenated format** (e.g., 'web-design' not 'Web Design')",
	);
	sections.push(`3. **Generate ${maxTagsPerItem} tags maximum** per item`);
	sections.push(
		"4. **Be specific** - 'react-hooks' is better than just 'react'",
	);
	sections.push(
		"5. **Consider the domain** as context for what the content is about",
	);
