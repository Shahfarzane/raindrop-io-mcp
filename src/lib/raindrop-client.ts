...
export async function getTags(collectionId?: number): Promise<TagsResponse> {
...
export async function suggestForUrl(
	link: string,
...
export async function getFeaturedCovers(): Promise<CoversResponse> {
	return apiRequest<CoversResponse>("/collections/covers");
}

export async function searchCovers(text: string): Promise<CoversResponse> {
	return apiRequest<CoversResponse>(
		`/collections/covers/${encodeURIComponent(text)}`,
	);
}
